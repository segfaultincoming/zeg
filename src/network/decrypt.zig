...
pub fn decrypt(packet: []const u8, decrypt_keys: keys.Keys) ![]const u8 {
    switch (packet[0]) {
        // 0xC1 and 0xC2 packets are not encrypted
        0xC1, 0xC2 => return packet,
        else => {},
    }

    // TODO: Counter is uknow to me right now. Works by only setting it here, though...
