...
pub const Context = struct {
    ...
    handle_packets: *const fn (server: *const Server, client: posix.socket_t, context: Context) void,
};
...
pub const Server = struct {
    ...
    pub fn listen(self: *const Server, context: Context, pool: *std.Thread.Pool) !void {
        while (true) {
            ...
            try pool.spawn(handle_packets, .{ self, client, context });
        }
    }
    ...
    fn handle_packets(server: *const Server, client: posix.socket_t, context: Context) void {
        context.handle_packets(server, client, context);
    }
};
